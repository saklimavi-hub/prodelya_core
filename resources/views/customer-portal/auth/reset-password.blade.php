    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-10">
        <div class="w-full max-w-lg rounded-2xl border border-stone-200 bg-white p-8 shadow-sm">
            <div class="mb-8 text-center">
                ...
                <h1 class="text-2xl font-semibold text-stone-900">Şifrenizi Yenileyin</h1>
                <p class="mt-2 text-sm text-stone-600">{{ $tenant->name }} müşteri portalı hesabınız için yeni şifre belirleyin.</p>
                <p class="text-sm text-stone-500">{{ $portalUser->safeDisplayName() }}</p>
            </div>
            ...
            <form method="POST" action="{{ route('customer.password.update', ['token' => $token]) }}" class="space-y-5">
                @csrf
                <div>
                ...
                <button type="submit" class="w-full rounded-xl bg-stone-900 px-4 py-3 text-sm font-medium text-white transition hover:bg-stone-800">Şifreyi Güncelle</button>
            </form>
        </div>
    </div>
</body>
