...
<style>
    ...
    @media (max-width: 1100px){.pqux-layout{grid-template-columns:1fr}.pqux-actions{justify-content:flex-start}.pqux-band-top,.pqux-decision{flex-direction:column}}
    @media (max-width: 720px){.pqux-summary-grid,.pqux-approval-grid{grid-template-columns:1fr}.pqux-item-top,.pqux-print{grid-template-columns:1fr}.pqux-totals{min-width:0}.pqux-history-row{flex-direction:column}.pqux-actions .pd-btn,.pqux-cta-row .pd-btn,.pqux-action-buttons .pd-btn{width:100%;justify-content:center}.pqux-action-buttons form{width:100%}}
</style>
        ...
        <div class="pqux-stack">
            <section class="pqux-card" data-testid="quote-send-actions-card">
                ...
                <div class="pqux-action-list">
                    ...
                    <div class="pqux-action-box">
                        <strong>Public Onay Linki</strong>
                        <span>{{ $approvalHelperUrl ? 'Aktif onay linki güvenli helper üzerinden açılabilir.' : 'Onay linki için önce geçerli bir gönderim oluşturulmalı.' }}</span>
                    </div>
                    <div class="pqux-action-box">
                ...
                <div class="pqux-action-buttons">
                    ...
                    @if($approvalHelperUrl)
                        <a href="{{ $approvalHelperUrl }}" class="pd-btn pd-btn-light" data-testid="quote-open-approval-link-button">Onay Linkini Aç</a>
                    @endif
                                ...
                                <div class="pqux-print-list">
                                    @foreach($visiblePrints as $printIndex => $print)
                                        <div class="pqux-print">
                                            ...
                                            <div>
                                                <div class="pqux-print-title">{{ trim(collect([$print->print_type, $print->print_option])->filter()->implode(' ')) }}</div>
        ...
        <div class="pqd-modal-panel" role="dialog" aria-modal="true" aria-labelledby="quote-send-modal-title">
            ...
            <form method="POST" action="{{ route('admin.promotion-quotes.send-to-customer', $quote) }}" class="mt-4">
                ...
                <div class="pqd-modal-grid">
                    ...
                    <div class="pqd-box">
                        <div class="pqd-label">Telefon</div>
                        <input type="text" name="contact_phone" value="{{ old('contact_phone', $quote->customer?->mobile ?: $quote->customer?->phone) }}" class="mt-2 w-full border border-gray-300 rounded px-3 py-2">
                    </div>
                </div>
