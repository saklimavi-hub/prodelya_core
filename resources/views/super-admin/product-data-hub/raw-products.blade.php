...
@section('title', 'Ham Ürün Havuzu')

@section('content')
<div class="pd-page-header">
    <div>
        <h1 class="pd-section-title">Ham Ürün Havuzu</h1>
...
<div class="pd-note mb-4">Bu ekran Super Admin tarafından yönetilir. Tenant bu verileri değiştiremez.</div>

@if($products->isNotEmpty())
    <div class="pd-card mb-6">
        <div class="pd-card-header"><strong>Kaynak Bazlı Toplu Dönüşüm</strong></div>
                ...
                @foreach($products->groupBy('supplier_source_id') as $sourceId => $sourceProducts)
                    ...
                    @if($source)
                        <form action="{{ route('admin.super.product-data-hub.sources.build-standard-products', $source) }}" method="POST">
                            @csrf
                            <button type="submit" class="pd-btn pd-btn-primary">{{ $source->source_name }} kaynağını standart ürünlere dönüştür</button>
                        </form>
                    @endif
                @endforeach
            </div>
...
<div class="pd-card">
    <div class="pd-card-body">
        @if($products->isEmpty())
            <div class="pd-note">Henüz ham ürün staging kaydı yok. Önce bir tedarikçi kaynağında Preview → Staging’e Aktar yapın.</div>
        @else
                        ...
                        @foreach($products as $product)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <form action="{{ route('admin.super.product-data-hub.raw-products.build-standard', $product) }}" method="POST">
                                            @csrf
...
<div>
    <strong>Ham ürün aksiyonları:</strong>
    <span class="pd-muted">Ham ürünleri standart ürüne dönüştürüp katalog projeksiyonuna hazırlayın.</span>
</div>
<div class="pd-bottom-action-buttons">
    <span class="pd-btn pd-btn-primary pd-btn-disabled">Standart Ürüne Dönüştür</span>
    ...
    <form action="{{ route('admin.catalog.project') }}" method="POST">
        @csrf
        <button type="submit" class="pd-btn pd-btn-warning">Katalog Projeksiyonu</button>
    </form>
</div>
@endsection
