...
class ProductDataHubVariantPriceStockFreshnessTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_etkin_0506_l_variant_change_updates_raw_standard_catalog_and_quote_search(): void
    {
        [$tenant, $source] = $this->makeTenantAndSource();
        $this->prepareJsonFixtureForSource($source, $this->etkin0506FeedRows());
        ...
        $rawProduct = $this->createEtkinRawProduct($source, [
            ...
            'normalized_payload' => [
                ...
                'stock_quantity' => 7800,
                'total_variant_stock_quantity' => 115300,
            ],
            'import_hash' => 'et-0506-parent',
        ]);

        $rawVariantS = $this->createEtkinRawVariant($source, $rawProduct, [
            'parent_supplier_product_id' => 4514,
            'supplier_group_code' => '0506',
            'variant_code' => '0506-S',
            'variant_stock_code' => '0506-S',
            'generated_variant_code' => 'ET-0506-S',
            'variant_name' => '0506-S Plastik Kalem',
            'variant_color' => 'Siyah',
            'variant_stock_quantity' => 7800,
            'normalized_payload' => [
                'list_price' => 6.5,
                'purchase_price' => 6.5,
                'currency' => 'TL',
            ...
            'import_hash' => 'et-0506-s',
        ]);

        $rawVariantL = $this->createEtkinRawVariant($source, $rawProduct, [
        ...
        $standardVariantS = StandardProductVariant::query()
            ...
            ->where('generated_variant_code', 'ET-0506-S')
            ->firstOrFail();
        $standardVariantL = StandardProductVariant::query()
            ->where('standard_product_id', $standardProduct->id)
            ->where('generated_variant_code', 'ET-0506-L')
            ->firstOrFail();

        $rawVariantS->forceFill(['standard_product_variant_id' => $standardVariantS->id])->save();
        $rawVariantL->forceFill(['standard_product_variant_id' => $standardVariantL->id])->save();
        ...
        $catalogVariant = TenantCatalogProductVariant::query()
            ->where('tenant_account_id', $tenant->id)
            ->where('standard_product_variant_id', $standardVariantL->id)
            ->firstOrFail();

        $this->assertSame(9.2, (float) data_get($rawVariantL->normalized_payload, 'list_price'));
        $this->assertSame(49800.0, (float) $rawVariantL->variant_stock_quantity);
        $this->assertSame(9.2, (float) $standardVariantL->min_purchase_price);
        $this->assertSame(49800.0, (float) $standardVariantL->stock_quantity);
        $this->assertSame(9.2, (float) $standardProduct->min_purchase_price);
        $this->assertSame(9.2, (float) $catalogVariant->display_price);
        $this->assertSame(49800.0, (float) $catalogVariant->stock_quantity);
        $this->assertSame(49800.0, (float) $catalogVariant->supplier_stock_quantity);
        $this->assertSame(0.0, (float) $catalogVariant->local_stock_quantity);
        $this->assertGreaterThanOrEqual(1, (int) data_get($run->report_payload, 'delta_apply_summary.dirty_standard_products_projected'));
    ...
    public function test_project_dirty_repairs_stale_variant_catalog_rows_when_standard_variants_are_already_current(): void
    {
        [$tenant, $source] = $this->makeTenantAndSource();
        $this->prepareJsonFixtureForSource($source, $this->etkin0506FeedRows());
        ...
        $rawProduct = $this->createEtkinRawProduct($source, [
            ...
            'normalized_payload' => [
                ...
                'pricing_policy_type' => 'list_price_only',
                'stock_quantity' => 40300,
                'total_variant_stock_quantity' => 90100,
            ],
            'import_hash' => 'et-0506-parent-current',
        ]);
        ...
        $catalogVariantS = TenantCatalogProductVariant::query()
            ->where('tenant_account_id', $tenant->id)
            ->where('standard_product_variant_id', $standardVariantS->id)
            ->firstOrFail();
        $catalogVariantL = TenantCatalogProductVariant::query()
            ->where('tenant_account_id', $tenant->id)
            ->where('standard_product_variant_id', $standardVariantL->id)
            ->firstOrFail();
        ...
        $catalogProduct->forceFill([
            'display_price' => 6.5,
            'supplier_stock_quantity' => 115300,
            'stock_quantity' => 115300,
            'updated_at' => $oldTimestamp,
        ])->save();
        $catalogVariantS->forceFill([
            'display_price' => 6.5,
            'supplier_stock_quantity' => 7800,
            'stock_quantity' => 7800,
            'updated_at' => $oldTimestamp,
        ])->save();
        $catalogVariantL->forceFill([
            'display_price' => 6.5,
            'supplier_stock_quantity' => 107500,
            'stock_quantity' => 107500,
            'updated_at' => $oldTimestamp,
        ])->save();

        $apply = app(SupplierSourceSyncService::class)->syncSource($source, [
            'run_type' => 'manual',
            'mode' => 'delta',
            'apply_price_stock' => true,
            'project_dirty' => true,
        ...
        $catalogVariantS->refresh();
        $catalogVariantL->refresh();

        $this->assertSame(0, (int) data_get($run->report_payload, 'delta_apply_summary.price_stock_applied'));
        $this->assertGreaterThanOrEqual(1, (int) data_get($run->report_payload, 'delta_apply_summary.dirty_standard_products_projected'));
        $this->assertGreaterThanOrEqual(2, (int) data_get($run->report_payload, 'delta_apply_summary.tenant_catalog_variants_updated'));
        $this->assertSame(9.2, (float) $catalogProduct->display_price);
        $this->assertSame(9.2, (float) $catalogVariantS->display_price);
        $this->assertSame(40300.0, (float) $catalogVariantS->supplier_stock_quantity);
        $this->assertSame(9.2, (float) $catalogVariantL->display_price);
        $this->assertSame(49800.0, (float) $catalogVariantL->supplier_stock_quantity);
    }

    private function makeTenantAndSource(): array
    {
        $tenant = TenantAccount::query()->create([
            'name' => 'Saklimavi Test',
            'legal_name' => 'Saklimavi Test Ltd.',
            'slug' => 'saklimavi-test-' . uniqid(),
            'panel_subdomain' => 'saklimavi-test-' . uniqid(),
            'status' => 'active',
            'default_locale' => 'tr',
        ...
        $supplier = Supplier::query()->create([
            'name' => 'Etkin Variant Freshness Supplier',
            'code' => 'ETKIN-VF-' . uniqid(),
            'status' => 'active',
        ]);
        ...
        $source = SupplierSource::query()->create([
            ...
            'source_type' => 'api',
            'source_name' => 'Etkin Variant Freshness Source',
            'status' => 'active',
            'config' => [
                'format' => 'json',
                'profile_key' => 'ETKIN',
    ...
    private function etkin0506FeedRows(): array
    {
        return [
            [
                'urun_id' => 4514,
                ...
                'urun_baslik' => '0506-S Plastik Kalem',
                ...
                'resim1' => 'https://example.test/0506_siyah.jpg',
                'md5' => 'hash-0506-s',
            ],
            [
                'urun_id' => 4516,
    ...
    {
        $fixturesDir = storage_path('framework/testing/product-data-hub-variant-freshness-fixtures');
