...
class SupplierSourceSyncService
{
    ...
    private function applyDeltaToRawProduct(SupplierProductRaw $product, array $current): void
    {
        ...
        $normalized = array_merge($normalized, [
            ...
            'source_list_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price'),
            'source_net_price' => $current['net_price'] ?? data_get($current, 'normalized_payload.net_price'),
            'source_purchase_price' => $current['purchase_price'] ?? data_get($current, 'normalized_payload.purchase_price'),
            'currency_origin' => $current['currency_origin'] ?? data_get($current, 'normalized_payload.currency_origin', $normalized['currency_origin'] ?? null),
            'currency_status' => $current['currency_status'] ?? data_get($current, 'normalized_payload.currency_status', $normalized['currency_status'] ?? null),
    ...
    private function applyDeltaToRawVariant(SupplierProductVariantRaw $variant, array $current): void
    {
        ...
        $normalized = array_merge($normalized, [
            ...
            'discount_rate' => $current['discount_rate'] ?? data_get($current, 'normalized_payload.discount_rate'),
            'currency' => $current['currency'] ?? data_get($current, 'normalized_payload.currency'),
            'source_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price') ?? data_get($normalized, 'source_price'),
            'source_currency' => $current['currency'] ?? data_get($current, 'normalized_payload.currency') ?? data_get($normalized, 'source_currency'),
            'source_list_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price'),
            'source_net_price' => $current['net_price'] ?? data_get($current, 'normalized_payload.net_price'),
            'source_purchase_price' => $current['purchase_price'] ?? data_get($current, 'normalized_payload.purchase_price'),
            'currency_origin' => $current['currency_origin'] ?? data_get($current, 'normalized_payload.currency_origin', $normalized['currency_origin'] ?? null),
    ...
    private function applyDeltaToStandardProduct(SupplierProductRaw $rawProduct, array $current): void
    {
        ...
        $sourceSummary = collect($product->source_summary ?? [])
            ->map(function (array $summary) use ($rawProduct, $current) {
                ...
                return array_merge($summary, [
                    ...
                    'source_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price', $summary['source_price'] ?? null),
                    'source_currency' => $current['currency'] ?? data_get($current, 'normalized_payload.currency', $summary['source_currency'] ?? null),
                    'source_list_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price', $summary['source_list_price'] ?? null),
                    'source_net_price' => $current['net_price'] ?? data_get($current, 'normalized_payload.net_price', $summary['source_net_price'] ?? null),
                    'source_purchase_price' => $current['purchase_price'] ?? data_get($current, 'normalized_payload.purchase_price', $summary['source_purchase_price'] ?? null),
                    'currency_origin' => $current['currency_origin'] ?? data_get($current, 'normalized_payload.currency_origin', $summary['currency_origin'] ?? null),
                    'currency_status' => $current['currency_status'] ?? data_get($current, 'normalized_payload.currency_status', $summary['currency_status'] ?? null),
        ...
        $priceSnapshot = array_merge((array) ($meta['price_snapshot'] ?? []), [
            ...
            'source_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price', $meta['price_snapshot']['source_price'] ?? null),
            'source_currency' => $current['currency'] ?? data_get($current, 'normalized_payload.currency', $meta['price_snapshot']['source_currency'] ?? null),
            'source_list_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price', $meta['price_snapshot']['source_list_price'] ?? null),
            'source_net_price' => $current['net_price'] ?? data_get($current, 'normalized_payload.net_price', $meta['price_snapshot']['source_net_price'] ?? null),
            'source_purchase_price' => $current['purchase_price'] ?? data_get($current, 'normalized_payload.purchase_price', $meta['price_snapshot']['source_purchase_price'] ?? null),
            'currency_origin' => $current['currency_origin'] ?? data_get($current, 'normalized_payload.currency_origin', $meta['price_snapshot']['currency_origin'] ?? null),
            'currency_status' => $current['currency_status'] ?? data_get($current, 'normalized_payload.currency_status', $meta['price_snapshot']['currency_status'] ?? null),
        ...
        $meta['normalized_payload'] = array_merge((array) ($meta['normalized_payload'] ?? []), [
            ...
            'discount_rate' => $current['discount_rate'] ?? data_get($current, 'normalized_payload.discount_rate'),
            'currency' => $current['currency'] ?? data_get($current, 'normalized_payload.currency'),
            'source_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price'),
            'source_currency' => $current['currency'] ?? data_get($current, 'normalized_payload.currency'),
            'source_list_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price'),
            'source_net_price' => $current['net_price'] ?? data_get($current, 'normalized_payload.net_price'),
            'source_purchase_price' => $current['purchase_price'] ?? data_get($current, 'normalized_payload.purchase_price'),
            'currency_origin' => $current['currency_origin'] ?? data_get($current, 'normalized_payload.currency_origin'),
            'currency_status' => $current['currency_status'] ?? data_get($current, 'normalized_payload.currency_status'),
            'vat_rate' => $current['vat_rate'] ?? data_get($current, 'normalized_payload.vat_rate'),
            'pricing_policy_type' => $current['pricing_policy_type'] ?? data_get($current, 'normalized_payload.pricing_policy_type'),
        ]);
    ...
    private function applyDeltaToStandardVariant(SupplierProductVariantRaw $rawVariant, array $current): ?int
    {
        ...
        $sourceSummary = array_merge((array) ($variant->source_summary ?? []), [
            ...
            'source_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price', $variant->source_summary['source_price'] ?? null),
            'source_currency' => $current['currency'] ?? data_get($current, 'normalized_payload.currency', $variant->source_summary['source_currency'] ?? null),
            'source_list_price' => $current['list_price'] ?? data_get($current, 'normalized_payload.list_price', $variant->source_summary['source_list_price'] ?? null),
            'source_net_price' => $current['net_price'] ?? data_get($current, 'normalized_payload.net_price', $variant->source_summary['source_net_price'] ?? null),
            'source_purchase_price' => $current['purchase_price'] ?? data_get($current, 'normalized_payload.purchase_price', $variant->source_summary['source_purchase_price'] ?? null),
            'currency_origin' => $current['currency_origin'] ?? data_get($current, 'normalized_payload.currency_origin', $variant->source_summary['currency_origin'] ?? null),
            'currency_status' => $current['currency_status'] ?? data_get($current, 'normalized_payload.currency_status', $variant->source_summary['currency_status'] ?? null),
